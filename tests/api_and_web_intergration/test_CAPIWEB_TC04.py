...
"""test_CAPIWEB_TC04 - Verify API-generated multiline task content on web."""


@pytest.mark.api
@pytest.mark.integration
@pytest.mark.regression
@pytest.mark.tc_id("test_CAPIWEB_TC04")
@pytest.mark.tc_title("Verify API-generated multiline task content on web.")
def test_verify_api_generated_multiline_task_contents_on_web(
    api_request_context,
    authenticated_page,
):
    ...
    project_name = unique_text("API WEB Multiline Project")
    task_title = unique_text("API WEB Multiline Task")
    task_content = (
        unique_text("Line one")
        + "\nLine two from API"
        + "\nLine three from API"
    )
