...
"""test_CWEB_TC11 - Verify long project name appears and selects."""


@pytest.mark.api
@pytest.mark.regression
@pytest.mark.tc_id("test_CWEB_TC11")
@pytest.mark.tc_title("Verify project with long name appears and full name appears in header.")
def test_verify_long_project_name_selects(api_request_context, authenticated_page):
    project_api = ProjectOrganize(api_request_context)
    task_api = TaskOrganize(api_request_context)
    project_ids = []
    task_records = []
    project_name = unique_text("PROJ WEB Long Name 1234567890")
